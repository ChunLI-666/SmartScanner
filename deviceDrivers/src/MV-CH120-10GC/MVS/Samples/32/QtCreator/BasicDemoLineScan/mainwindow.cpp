...
void MainWindow::on_SelchangeTriggersourceCombo_currentTextChanged(const QString &arg1)
{
    ...
    for (auto it = m_mapTriggerSource.begin(); it != m_mapTriggerSource.end(); it++)
    {
        if (it.key() == arg1)
        {
...
{
    for (auto it = m_mapPixelFormat.begin(); it != m_mapPixelFormat.end(); it++)
    {
        if (it.key() == arg1)
        {
...
{
    for (auto it = m_mapPreampGain.begin(); it != m_mapPreampGain.end(); it++)
    {
        if (it.key() == arg1)
        {
