...
public:
    ...
    int Init(int nBufCount, int DefaultImagelen)
    {
        ...
        this->Queue = new stImageNode[nBufCount];
        this->Qlen = nBufCount;
...
static void* WorkThread(void* pUser)
{
	...
	while(true != g_bExit)
	{
        ...
        {
            printf("Get nWidth_1 [%d] nHeight_1 [%d] nFrameNum [%d]  \r\n",nWidth,nHeight,nFrameNum);

             //根据实际场景需求，对图像进行 处理   
#if 0
            FILE* fp = NULL;
            char szFileName[256] = {0};
            sprintf(szFileName, "Image_%d_width_%d_height_%d_Len.raw",nFrameNum,nWidth,nHeight,nFrameLen);
