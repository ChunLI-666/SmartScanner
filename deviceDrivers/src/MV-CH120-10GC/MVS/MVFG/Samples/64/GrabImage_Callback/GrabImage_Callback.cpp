/* 
ch：该示例程序展示如何使用SDK设置内部缓存并使用MV_FG_RegisterFrameCallBack（）回调接口采集图像。在回调函数中不建议进行耗时操作，避免出现丢帧的情况。

en: This sample program shows how to use the SDK to set the internal buffer and use the callback API MV_ FG_ The RegisterFrameCallBack() to acquire images.
    It is not recommended to perform time-consuming operations in the callback function, which will lead to frame loss.
*/
