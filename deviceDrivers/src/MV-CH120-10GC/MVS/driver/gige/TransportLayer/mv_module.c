...
/*file_operations 结构体中 ioctl 在内核V2.6.36 之后被替换成 unlocked_ioctl compat_ioctl
  内核V2.6.36 对应Ubuntu 11.04 （不包括）之前版本 
  内核V2.6.36 对应Centos 6.0 （不包括）之前版本  
  如果对应之前版本编译失败，删除 HAVE_UNLOCKED_IOCTL 的定义即可
  
  高版本内核删除了 HAVE_UNLOCKED_IOCTL 的定义，导致走到了不支持的 ioctl 分支，导致异常。
 */
