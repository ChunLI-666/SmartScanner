...
// 驱动支持的设备
#ifdef _XOF
struct pci_device_id CxpDeviceId[] = {
    {CXP_VENDOR_ID, XOF_DEVICE_ID, CXP_VENDOR_ID, XOF_SUBDEVICE_ID, 0, 0, 0},
    {0,}
};
#else
struct pci_device_id CxpDeviceId[] = {
    {CXP_VENDOR_ID, CXP6_DEVICE_ID, CXP_VENDOR_ID, CXP6_SUBDEVICE_ID, 0, 0, 0},
    {CXP_VENDOR_ID, CXP12_DEVICE_ID, CXP_VENDOR_ID, CXP12_SUBDEVICE_ID, 0, 0, 0},
    {0,}
};
#endif

MODULE_DEVICE_TABLE(pci, CxpDeviceId);

struct pci_driver CxpDriver = {
    .name = MV_MODULE_LONG_NAME,
    .id_table = CxpDeviceId,
    .probe = ModuleProbe,
    .remove = ModuleRemove,
};

struct file_operations cxp_fops = {
    .owner          = THIS_MODULE,
    .open           = ModuleOpen,
...
int MV_AddDevice(void* pPciDevice, char* chDeviceName, char* chCxpClassName)
{
    ...
    cdev_init(&pstPciDevice->stCdev, &cxp_fops);
    // 将cdev和设备号关联起来
    ...
    pstPciDevice->pstClass = class_create(THIS_MODULE, (const char *)chCxpClassName);
...
int MV_RequestIrq(void* pPciDevice, irqreturn_t (*interrputfn)(int irq, void *data), void* pDriverData)
{
    ...
    do
    {
        ...
        {
            nRet = request_irq(pstDeviceData->hMSInterrupt[nIrqCount], interrputfn, IRQF_SHARED, MV_MODULE_LONG_NAME, pDriverData);
...
int ModuleProbe(struct pci_dev* pstPciDev, const struct pci_device_id* pstPciDeId)
{
    ...
    printk(KERN_INFO"[%s:%d]:Module Probe Enter\n", __FUNCTION__, __LINE__);
    do
    {
    ...
    if (MV_OK == nRet)
    {
        printk(KERN_INFO"[%s:%d]:Module Probe finish\n", __FUNCTION__, __LINE__);
        return nRet;
    }
    else
    {
...
#ifdef _XOF
    printk(KERN_INFO"[%s:%d]:XOF Driver Enter: Version[%s]\n", __FUNCTION__, __LINE__, MV_VERSION);
#else
    printk(KERN_INFO"[%s:%d]:CXP Driver Enter: Version[%s]\n", __FUNCTION__, __LINE__, MV_VERSION);
#endif

    // 注册pci driver
    nRet = pci_register_driver(&CxpDriver);
    if (nRet < 0) {
...
static void ExitModule(void)  
{
    ...
    pci_unregister_driver(&CxpDriver);
    printk(KERN_INFO"[%s:%d]:end\n", __FUNCTION__, __LINE__);
}
