...
int MV_GetUserPages( long* aContext, MV_MEMORYPAGELIST* aMap, void* aUserAddress, unsigned int aSize )
{
    ...
    if (( ( ( unsigned long ) aUserAddress ) + ( unsigned long ) aSize ) % PAGE_SIZE > 0)
    {
        nReqPageCount = lLastPage - lFirstPage + 1;
    }
    else
    {
        nReqPageCount = lLastPage - lFirstPage;
        if (nReqPageCount == 0)
        {
            nReqPageCount ++;
        }
    ...
    aMap->nUnSgMapPageCount = nReqPageCount;
    aMap->nSgMapPageCount = 0;
    aMap->nNeedMapPageCount = 0;
    aMap->nUnSgMapSize = aSize;
    aMap->nSgMapSize = 0;
    aMap->nLastPageSize = PAGE_SIZE;
    return 0;
}
...
//ͬһʱ�̿���ӳ�������ᱻ���ǣ�����V2.6.0����Ҫ�Լ�����km_type���ͣ�Ӧ�����User0��User1�������
...
//     {
//         printk("CxpDriver: vma is too big\n");
//         return -1;
//     }
...
//         if (0 != nRet)
//         {
//             printk("CxpDriver: remap_page_range failed ,nRet[%d], i[%d]\n", nRet, i);
//             return -1;
//         }
