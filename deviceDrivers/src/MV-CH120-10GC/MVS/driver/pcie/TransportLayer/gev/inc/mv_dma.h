...
extern struct scatterlist * MV_DmaSgGetNextSgElem(void* pSclist);

extern int MV_DmaSgSetSclistAddrOffset(void* pSclist, unsigned long nAddr);

extern int MV_DmaSgSetSclistLength(void* pSclist, int nSgElemLen);

extern int MV_DmaSgSetSgNents(PMV_DIR_MEM_ACCESS pstDma, int nNents);
