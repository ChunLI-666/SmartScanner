...
typedef struct _MV_MEMORYPAGELIST_
{
    ...
    unsigned long  nUnSgMapPageCount;
    unsigned long  nSgMapPageCount;
    unsigned long  nNeedMapPageCount;
    unsigned int   nUnSgMapSize;
    unsigned int   nSgMapSize;
} MV_MEMORYPAGELIST;
...
extern void* MV_PageAddress(const struct page *page);
