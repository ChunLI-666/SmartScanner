...
int MV_MutexInit(MV_MUTEX* pstMvMutex)
{
    struct mutex* pstMutex = NULL;
    ...
    pstMvMutex->pMutex = kmalloc(sizeof(struct mutex), GFP_KERNEL);
    if (NULL == pstMvMutex->pMutex)
    {
        return -ENOMEM;
    }
    
    pstMutex = (struct mutex *)(pstMvMutex->pMutex);
...
int MV_MutexFree(MV_MUTEX* pstMvMutex)
{
    if (pstMvMutex == NULL)
    {
        return -EINVAL;
    ...
    if (NULL != pstMvMutex->pMutex)
    {
        kfree((struct mutex *)pstMvMutex->pMutex);
        pstMvMutex->pMutex = NULL;
    }
    return MV_OK;
}


int MV_MutexLock(MV_MUTEX* pstMvMutex)
{
    struct mutex* pstMutex = NULL;
    if(NULL == pstMvMutex)
    {
...
int MV_MutexUnlock(MV_MUTEX* pstMvMutex)
{
    struct mutex* pstMutex = NULL;
    if(NULL == pstMvMutex)
    {
