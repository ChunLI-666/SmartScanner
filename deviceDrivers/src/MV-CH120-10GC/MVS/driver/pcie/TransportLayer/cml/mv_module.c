...
// 驱动支持的设备
struct pci_device_id CmlDeviceId[] = {
    {CML_VENDOR_ID, CML_DEVICE_ID, CML_SUBVENDOR_ID, CML_SUBDEVICE_ID, 0, 0, 0},
    {0,}
};

MODULE_DEVICE_TABLE(pci, CmlDeviceId);

struct pci_driver CmlDriver = {
    .name = MV_MODULE_LONG_NAME,
    .id_table = CmlDeviceId,
    .probe = ModuleProbe,
    .remove = ModuleRemove,
};

struct file_operations cml_fops = {
    .owner          = THIS_MODULE,
    .open           = ModuleOpen,
...
int MV_AddDevice(void* pPciDevice, char* chDeviceName, char* chCmlClassName)
{
    ...
    cdev_init(&pstPciDevice->stCdev, &cml_fops);
    // 将cdev和设备号关联起来
    ...
    pstPciDevice->pstClass = class_create(THIS_MODULE, (const char *)chCmlClassName);
...
int ModuleProbe(struct pci_dev* pstPciDev, const struct pci_device_id* pstPciDeId)
{
    ...
    if (unlikely(pci_request_regions(pstPciDev, MV_MODULE_LONG_NAME))) {
        printk(KERN_ERR"[%s:%d]:pci_request_regions failed!\n", __FUNCTION__, __LINE__);
        nRet = -EIO;
    ...
    if (pstDeviceData->pBar0Phy < 0) {
        printk(KERN_ERR"[%s:%d]:pci_resource_start failed!\n", __FUNCTION__, __LINE__);
        nRet = -EIO;
    ...
    if (nRequestIrqNum != MSI_COUNT) {
        printk(KERN_ERR"[%s:%d]:pci alloc irq vectors failed! Request irq number[%d]\n", __FUNCTION__, __LINE__, nRequestIrqNum);
        nRet = MV_E_RESOURCE;
...
static int InitModule(void)
{
    ...
    printk(KERN_INFO"[%s:%d]:CL Driver Enter: Version[%s]\n", __FUNCTION__, __LINE__, MV_VERSION);

    // 注册pci driver
    nRet = pci_register_driver(&CmlDriver);
    if (nRet < 0) {
...
static void ExitModule(void)  
{
    ...
    pci_unregister_driver(&CmlDriver);
    printk(KERN_INFO"[%s:%d][CL PCIE]:end\n", __FUNCTION__, __LINE__);
}
